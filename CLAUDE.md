...
- **Strava integration.** One shared Strava API app for all users: `config.js` carries `window.stravaConfig` (`clientId` + `tokenEndpoint`), and the client secret lives only in the Cloudflare Worker under `strava-worker/` (see its README for deploy steps). OAuth via `startStravaOAuth` → redirect → `handleStravaOAuthCallback` → `exchangeStravaCodeForToken` (POSTs the code to the worker's `/exchange`). Tokens (`stravaAccessToken`, `stravaRefreshToken`, `stravaTokenExpiresAt`) are written to the user doc, so the callback only runs after sign-in (guarded by `stravaCallbackHandled`). Expired tokens are refreshed automatically via the worker's `/refresh` (`refreshStravaToken`, called proactively by `syncStravaActivities` and reactively on 401 in `fetchStravaActivities`). Activities are fetched (`fetchStravaActivities`), saved (`saveStravaActivitiesToDB`), and classified by sport via the `isRunningActivity` / `isWeightTrainingActivity` / `isYogaActivity` / `isColdPlungeActivity` / `isHikingActivity` / `isSkiActivity` predicates → `getActivityCategories` → `getActivityIcon` (emoji shown on the day cell).
